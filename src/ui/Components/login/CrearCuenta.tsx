...
export default function CrearCuenta() {
  ...
  const [firstName, setFirstName] = useState(""); // Cambiado a firstName
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.email == "Usuario with this email already exists.") {
          setErrorMessage("Ya exsite un usuario con este email");
        } else {
      ...
      if (error instanceof Error) {
        setErrorMessage(error.message ? errorMessage : "Ocurrió un error.");
      } else {
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="relative">
          ...
          />
          <UserIcon className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-500 peer-focus:text-green-500 pointer-events-none" />
        </div>
        ...
        <div className="relative">
          <input
            ...
            className="peer block w-full rounded-md border-b-2 border-gray-300 py-3 pl-10 placeholder:text-gray-500 focus:border-green-500 outline-none transition duration-200"
          />
          <IdentificationIcon className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-500 peer-focus:text-green-500 pointer-events-none" />
        </div>
